...
function DashboardLayout({ menuItems }) {
    ...
    const activeTab = menuItems.find(item => location.pathname.endsWith(item.path))?.label;
        ...
        <section className="flex flex-col md:flex-row">
            ...
            <div className="md:hidden p-4 border-b bg-white flex items-center justify-between">
                <div className="flex items-center gap-2">
                    <button onClick={() => setIsMobileOpen(!isMobileOpen)} className="text-2xl p-1 hover:bg-gray-100 rounded">
                        {isMobileOpen ? "❌" : "🍔"}
                    </button>
            ...
            <aside className={`shrink-0 bg-white border-b md:border-b-0 md:border-r border-gray-300 transition-all duration-300 ease-in-out overflow-hidden
            ...
            ${isMobileOpen ? "max-h-screen opacity-100" : "max-h-0 opacity-0 md:max-h-screen md:opacity-100"}`}>
                <div className="p-2">
                    ...
                    <div className="hidden md:block mb-2 text-right border-b border-gray-300">
                        <button onClick={() => setIsCollapsed(!isCollapsed)} className="p-2 text-xl">
                            {isCollapsed ? "➡️" : "⬅️"}
                        </button>
                    </div>
